...
export class CountryService {
	...
	private url = this.endpoint + 'classes/country';
	...
	getCount(){
		...
		return this.http.post(url, JSON.stringify({table_name: 'country'}), {headers: this.headers})
		.map((response: Response) => response.json());
	}

	update(id: number, data: any){
		let url = this.endpoint + 'classes/country/' + id;
